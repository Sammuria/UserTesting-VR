...
public class FirstPersonController : MonoBehaviour
{
    ...
    public Camera cam1;
    public Camera cam2;
    public KeyCode cam1Key = KeyCode.Alpha1;
    public KeyCode cam2Key = KeyCode.Alpha2;
    public KeyCode escape = KeyCode.Tab;
    public GameObject pauseScreen;
    public KeyCode shiftUp = KeyCode.Plus;
    public KeyCode shiftDown = KeyCode.Minus;
    ...
    {        
        ...
        // enabling and disabling main camera to switch player view
        if (Input.GetKeyDown(cam1Key))
        {
            cam2.enabled = false;
            // Debug.Log("Main camera status: " + thirdCamera.enabled);
            if (cam1.enabled)
            {
                cam1.enabled = false;
            }
        ...
        else if (Input.GetKeyDown(cam2Key))
        {
            cam1.enabled = false;
            // Debug.Log("Main camera status: " + thirdCamera.enabled);
            if (cam2.enabled)
        ...
        if (Input.GetKeyDown(shiftDown))
        {
            Camera cam = gameObject.GetComponentInChildren(typeof(Camera)) as Camera;
            cam.transform.position = cam.transform.position + new Vector3(0f, -0.2f, 0f);
        }
        else if (Input.GetKeyDown(shiftUp))
        {
            Camera cam = gameObject.GetComponentInChildren(typeof(Camera)) as Camera;
            cam.transform.position = cam.transform.position + new Vector3(0f, 0.2f, 0f);
        }

        //allow user to pause and free the mouse
        if (Input.GetKeyDown(escape) && cameraCanMove)
        {
            // Debug.Log("Pausing prototype...");
            cameraCanMove = false;
            Cursor.lockState = CursorLockMode.None;
            pauseScreen.SetActive(true);
        }
        else if (Input.GetKeyDown(escape) && !cameraCanMove)
        {
            // Debug.Log("Unpausing prototype...");
            cameraCanMove = true;
            Cursor.lockState = CursorLockMode.Locked;
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Edited for Breakout Bandits", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();
        ...
        GUILayout.Label("3rd Person Camera", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        fpc.shiftUp = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Shift Camera Up", "Determines what key is used to shift camera view."), fpc.shiftUp);
        fpc.shiftDown = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Shift Camera Down", "Determines what key is used to shift camera view."), fpc.shiftDown);
        fpc.cam1 = (Camera)EditorGUILayout.ObjectField(new GUIContent("Camera 1", "3rd person camera."), fpc.cam1, typeof(Camera), true);
        fpc.cam2 = (Camera)EditorGUILayout.ObjectField(new GUIContent("Camera 2", "3rd person camera."), fpc.cam2, typeof(Camera), true);
        fpc.cam1Key = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Camera 1 Key", "Determines what key is used to switch camera view."), fpc.cam1Key);
        fpc.cam2Key = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Camera 2 Key", "Determines what key is used to switch camera view."), fpc.cam2Key);

        EditorGUILayout.Space();
        fpc.escape = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Escape Key", "Determines what key is used to pause the testing."), fpc.escape);
        fpc.pauseScreen = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Pause Screen", "UI for pause screen."), fpc.pauseScreen, typeof(GameObject), true);
