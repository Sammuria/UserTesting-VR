...
/*
        var thumbStickaxis = OVRInput.Get(OVRInput.RawAxis2D.RThumbstick,OVRInput.Controller.RTouch);
        ...
        //transform.rotation = Quaternion.LookRotation(player.position);
        Debug.Log(thumbStickaxis.ToString());
        Debug.Log(player.transform.localRotation.eulerAngles.y);
        Debug.Log(player.transform.localRotation.eulerAngles.z);
...
*/
    ...
    void SetupTeleportDefaults()
    {
       ...
       // var orient = TeleportController.GetComponent<TeleportOrientationHandlerThumbstick>(); ss commented this to remove snap turns with tstick
       // orient.Thumbstick = OVRInput.Controller.LTouch; ss commented this to remove snap turns with tstick
    }
    ...
    void SetupNodeTeleport()
    {
        ...
        lc.PlayerController.RotationEitherThumbstick = false;  //SS changed to false
        //TeleportController.EnableRotation(true, false, false, true); //enables snap turns
        ...
        var input = TeleportController.GetComponent<TeleportInputHandlerTouch>();
        input.AimingController = OVRInput.Controller.RTouch; //Beam switches if this is commented
